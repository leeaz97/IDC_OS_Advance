...
void vm_init(struct vm *vm, size_t mem_size , int num_vcpu)
{
	int api_ver;
...
void vcpu_init(struct vm *vm, struct vcpu *vcpu, int index_vcpu)
{
	int vcpu_mmap_size;

	vcpu->fd = ioctl(vm->fd, KVM_CREATE_VCPU, index_vcpu);
        if (vcpu->fd < 0) {
...
static void setup_long_mode(struct vm *vm, struct kvm_sregs *sregs, uint64_t base_vcpu_address)
{
	// distinct guest page table for each of the guest instances - add to the page table the first address of guest 
	uint64_t pml4_addr = base_vcpu_address + 0x2000;
	uint64_t *pml4 = (void *)(vm->mem + pml4_addr);

	uint64_t pdpt_addr = base_vcpu_address + 0x3000;
	uint64_t *pdpt = (void *)(vm->mem + pdpt_addr);

	uint64_t pd_addr = base_vcpu_address + 0x4000;
...
int run_long_mode(struct vm *vm, struct vcpu *vcpu, uint64_t base_vcpu_address)
{
	...
	setup_long_mode(vm, &sregs, base_vcpu_address);
	...
	memcpy(vm->mem + base_vcpu_address, guest64, guest64_end-guest64);
...
int main(int argc, char **argv)
{
	...
	struct thread_for_vcpu thread_vcpu;
	pthread_t threadId;
	...
	int num_vm =2;
	// double the size of the guest physical memory 
	vm_init(&vm, 0x400000, num_vm);

	vcpu_init(&vm, &vcpu1, 0);
